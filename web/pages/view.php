...
if (isset($_GET['section'])) {
    ...
    if ($section != '') {
        ...
        switch ($section) {
            case 'admin':
                $urlFinal = "/app/view/Admin/" . $pageAction;
                break;

            case 'uploads':
                $urlFinal = "/app/view/Upload/" . $pageAction;
                break;

            default:
                $urlFinal = "/app/view/" . $model . "/" . $pageAction;
                break;
        }

        try {
            Utils::includeFileStatic($urlFinal);
//            $util->includeFile($urlFinal);
        } catch (Exception $err) {
            echo $err->getMessage();
        }
    }
